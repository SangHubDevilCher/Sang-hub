...
local serv = win:Server("Sang Hub (BETA-3)","")
...
AutoFarm:Toggle("Cày cấp(level" đơ nát máy)", _G.AutoFarmLevel, function(vu)
    _G.AutoFarm = vu
...
local SelectWeapon = AutoFarm:Dropdown("Chọn vũ khí(weapons)",Wapon,function(Value)
    WeaponMastery = Value
end)

AutoFarm:Button("Reset(nhìn cặc)", function()
    SelectWeapon:Clear()
    for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
        if v:IsA("Tool") then
            SelectWeapon:Add(v.Name)
...
local Tp = serv:Channel("Di Chuyển(tp)","http://www.roblox.com/asset/?id=7044226690")
...
local Raid = serv:Channel("Esp-Tập Kích(raid)","http://www.roblox.com/asset/?id=7251993295")
if New_World then
...
Raid:Toggle("(tự động)Nâng chiêu v2", false, function(vu)
    AutoAwakener = vu
end)

Raid:Toggle("Qua đảo(pháp sư trung hoa)", false, function(vu)
    NextIsland = vu
end)

Raid:Line()

Raid:Toggle("Tự động tập kích(auto raid)", false, function(vu)
    _G.AutoRaid = vu
end)
...
Raid:Toggle("Định vị Người chơi(vô gia cư)",false,function(Value)
    PlayerESP = Value
	while PlayerESP do wait()
		UpdatePlayer()
	end
end)
Raid:Toggle("Định vị rương(bạc tỉ)!",false,function(Value)
    ChestESP = Value
	while ChestESP do wait()
        UpdateChest()
    end
end)
Raid:Toggle("Định vị trái ác quỷ(fruit)",false,function(Value)
    DevilESP = Value
...
if New_World then
	Raid:Toggle("Định vị hoa(florentino)",false,function(Value)
		FlowerESP = Value
...
local BuyItem = serv:Channel("Cửa hàng(shop)","http://www.roblox.com/asset/?id=7294901968")
...
local Misc = serv:Channel("Khác(more)","http://www.roblox.com/asset/?id=7044233235")
...
Misc:Button("Cửa hàng trái(shop stock)", function()
    local args = {
        [1] = "GetFruits"
    }
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
...
local info = serv:Channel("info|tik tok:zennos._ hoặc sangneba|youtube: https://www.youtube.com/@DuckBunny (duckbunny)")